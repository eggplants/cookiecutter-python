...
def test_distroless_switches_the_runtime_base(bake):
    ...
    readme = (bake(use_distroless="yes") / "README.md").read_text(encoding="utf-8")
    assert "The image is distroless" in readme
    assert "distroless" not in (bake() / "README.md").read_text(encoding="utf-8")
...
@pytest.mark.parametrize(
    ...
    [("cli", ["Install", "CLI", "Library", "License"]), ("library", ["Install", "Library", "License"])],
)
...
def test_install_matches_what_the_project_ships(bake):
    ...
    assert "my-python-project-linux-amd64" in binaries
