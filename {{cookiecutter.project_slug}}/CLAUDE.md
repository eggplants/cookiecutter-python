...
uv run pytest tests/test_{{ cookiecutter.package_name }}.py::test_version_is_available  # a single test
...
- **`{{ cookiecutter.package_name }}/__init__.py`** -- package version, read from the installed
  distribution metadata (`0.0.0` when running from a source tree with no tags).
{%- if cookiecutter.project_type == "cli" %}
- **`{{ cookiecutter.package_name }}/cli.py`** -- argparse entry point (`{{ cookiecutter.command_name }}`).
  `main()` takes an optional argument list so the tests can drive it without touching `sys.argv`.
- **`{{ cookiecutter.package_name }}/__main__.py`** -- makes `python -m {{ cookiecutter.package_name }}` work.
{%- endif %}
...
then reacts to `release: [published]` and does the PyPI{% if cookiecutter.use_docker == "yes" %} and GHCR{% endif %} publish.
{%- else %}
Pushing a `v*.*.*` tag runs `release.yml`, which creates the GitHub release and then publishes to
PyPI{% if cookiecutter.use_docker == "yes" and cookiecutter.project_type == "cli" %} and GHCR{% endif %}.
{%- endif %}
